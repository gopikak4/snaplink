  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: '#080c10', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '48px', maxWidth: '500px' }}>
        ...
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '32px', marginBottom: '8px' }}>Create account ✨</h1>
        <p style={{ color: '#6e7681', fontSize: '15px', marginBottom: '28px' }}>Start shortening links for free today</p>
        ...
        <div style={{ background: '#0d1117', border: '1px solid #21262d', borderRadius: '20px', padding: '28px', boxShadow: '0 0 60px rgba(34,197,94,0.06)' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {/* Name */}
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#8b949e', marginBottom: '8px', fontWeight: 600, letterSpacing: '0.5px' }}>FULL NAME</label>
              <input type="text" placeholder="Your name" value={form.name}
                onChange={e => { setForm({ ...form, name: e.target.value }); setRobotMood(e.target.value ? 'name' : 'idle') }}
                style={{ width: '100%', background: '#161b22', border: `1px solid ${errors.name ? '#f87171' : '#30363d'}`, borderRadius: '12px', padding: '14px 16px', color: '#fff', fontSize: '14px', outline: 'none', boxSizing: 'border-box', transition: 'all 0.2s' }}
                onFocus={e => { e.target.style.borderColor = '#22c55e'; e.target.style.boxShadow = '0 0 0 3px rgba(34,197,94,0.1)'; setRobotMood('name') }}
                onBlur={e => { e.target.style.borderColor = errors.name ? '#f87171' : '#30363d'; e.target.style.boxShadow = 'none' }}
              />
            ...
            <div>
              ...
              <input type="email" placeholder="you@example.com" value={form.email}
                onChange={e => { setForm({ ...form, email: e.target.value }); setRobotMood(e.target.value ? 'typing' : 'idle') }}
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input type={showPass ? 'text' : 'password'} placeholder="••••••••" value={form.password}
                  onChange={e => { setForm({ ...form, password: e.target.value }); setRobotMood('password') }}
            ...
            <button type="submit" disabled={loading}
              ...
              {loading
                ? <><div style={{ width: '18px', height: '18px', border: '2px solid #080c10', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} /><span>Creating account...</span></>
